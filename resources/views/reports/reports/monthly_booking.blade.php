...
@section('content')
                        ...
                        <tbody>
                            @foreach ($bookings as $booking)
                                @php
                                    ...
                                    $rooms = $booking->booking_details->units_count ?? 1;
                                    $nights = $booking->days_count ?? 1;

                                    $totalCost = $roomCost * $rooms * $nights;
                                    $totalPrice = $sellingPrice * $rooms * $nights;
                                    ...
                                    if ($booking->commission_type === 'percentage') {
                                        ...
                                        $commissionAmount = $booking->commission_night * $rooms * $nights;
                                    }

                                    // Broker total if exists
                                    $total_broker = 0;
                                    if ($booking->broker_amount) {
                                        $total_broker = $booking->broker_amount * $rooms * $nights;
                                    }
                                @endphp
                                <tr>
    ...
    <script>
        ...
        function exportToExcel() {
            ...
            exportForm.action = "{{ route('booking.export') }}";
