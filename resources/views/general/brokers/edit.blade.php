...
@section('title')
    {{ __('roles.broker_managment') }}
@endsection
@section('content')
    <div class="page-breadcrumb">
        <div class="row">
            <div class="col-5 align-self-center">
                <h4 class="page-title">{{ __('roles.broker_managment') }}</h4>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-control" value="{{ $broker->name }}">
                                @error('name')
                            ...
                            <div class="form-group">
                                <label for="">{{ __('general.nationality') }} <span class="text-danger">*</span></label>
                                <select name="nationality_id" class="form-control js-select2-custom ">
                                    @foreach ($countries as $countries_item)
                                    <option value="{{ $countries_item->id }}"  {{ ($countries_item->id == $broker->country_id) ? 'selected' : '' }}>{{ $countries_item->name }}</option>
                                    @endforeach
                                </select>
