                ...
                <div class="card-body">
                    @if($file->isNotEmpty())
                        <div class="customer-info">
                            ...
                            <p><strong>{{ __('Phone') }}:</strong> {{ $file->first()->customer->phone ?? 'N/A' }}</p>
                        </div>
                    @endif
                    ...
                    <div class="card file-card">
                        <div class="file-header">
                            ...
                            <span class="badge badge-status
                                ...
                                @if($custFile->remain == 0) {{ __('Fully Paid') }}
                                @elseif($custFile->paid > 0) {{ __('Partially Paid') }}
                                @else {{ __('Not Paid') }} @endif
                            </span>
                        </div>
                            ...
                            <div class="row">
                                    ...
                                    <div class="card item-card mb-3">
                                        <div class="card-body">
                                                ...
                                                <div class="mb-2">
                                                    ...
                                                    <div class="item-details">
                                                        <small class="text-muted">
                                                            {{ __('Total') }}: ${{ number_format($booking->total ?? 0, 2) }}<br>
                                                            {{ __('Check In') }}: {{ $booking->arrival_date ?? 'N/A' }}<br>
                                                            {{ __('Check Out') }}: {{ $booking->check_out_date ?? 'N/A' }}
                                                        </small>
                                                    </div>
                                                </div>
                                            @elseif($item->related_type === 'App\\Models\\Car')
                                                ...
                                                <div>
                                                    <strong>{{ __('Car') }}:</strong> #{{ $car->id ?? 'N/A' }}
                                                    <div class="item-details">
                                                        <small class="text-muted">
                                                            {{ __('Model') }}: {{ $car->category->category ?? 'N/A' }}<br>
                                                            {{ __('Price') }}: ${{ number_format($car->total ?? 0, 2) }}
                                                        </small>
                                                    </div>
                                                </div>
                                            @endif
                                ...
                                <div class="col-md-6">
                                    <div class="payment-summary">
                                        ...
                                        <div class="row mb-2">
                                            <div class="col-6 detail-label">{{ __('Total Amount') }}:</div>
                                            <div class="col-6 text-right detail-value">${{ number_format($custFile->total, 2) }}</div>
                                        </div>
                                        <div class="row mb-2">
                                            <div class="col-6 detail-label">{{ __('Paid Amount') }}:</div>
                                            <div class="col-6 text-right detail-value">${{ number_format($custFile->paid, 2) }}</div>
                                        </div>
                                        <div class="row mb-2">
                                            <div class="col-6 detail-label">{{ __('Remaining Balance') }}:</div>
                                            <div class="col-6 text-right detail-value">${{ number_format($custFile->remain, 2) }}</div>
                                        </div>
                                        <hr>
                                        <div class="row">
                                            <div class="col-6 detail-label">{{ __('Created At') }}:</div>
                                            <div class="col-6 text-right">
                                                <small class="text-muted">{{ $custFile->created_at->format('d M Y H:i') }}</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
